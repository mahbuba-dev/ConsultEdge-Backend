...
import { Role as Role3, PaymentStatus as PaymentStatus2 } from "@prisma/client";
...
var getAdminStats = async () => {
  ...
  const totalRevenueAgg = await prisma.payment.aggregate({
    _sum: { amount: true },
    where: { status: PaymentStatus2.PAID }
  });
...
var getExpertStats = async (user, filters = {}) => {
  ...
  const bookingsByMonth = await prisma.consultation.groupBy({
    by: [prisma.Prisma.sql`DATE_TRUNC('month', "createdAt")`],
    where: { expertId: expert.id, ...filters.consultation },
    _count: { id: true },
  ...
  let profileSaves = 0;
  try {
    profileViews = await prisma.expertProfileView.count({ where: { expertId: expert.id } });
    profileSaves = await prisma.savedExpert.count({ where: { expertId: expert.id } });
  } catch (e) {
  }
  return {
    consultationCount,
...
var getClientStats = async (user, filters = {}, pagination = {}) => {
  ...
  try {
    savedExpertsCount = await prisma.savedExpert.count({ where: { clientId: client.id } });
  } catch (e) {
  }
  const engagementTrends = await prisma.consultation.groupBy({
    by: ["createdAt"],
    where: { clientId: client.id, ...filters.consultation },
    _count: { id: true },
    orderBy: { createdAt: "asc" }
  });
  const spendingByMonth = await prisma.payment.groupBy({
    by: ["createdAt"],
    // Use createdAt and group in JS if needed
    where: {
      status: PaymentStatus2.PAID,
      clientId: client.id,
      ...filters.payment
  ...
  const categoryDistribution = await prisma.consultation.groupBy({
    by: ["industryId"],
    where: { clientId: client.id, ...filters.consultation },
    _count: { id: true }
  });
  let aiUsage = [];
  try {
    aiUsage = await prisma.aiUsageLog.groupBy({
      by: ["type"],
      // For time-based, group in JS if needed
      where: { userId: user.userId, ...filters.aiUsage },
      _count: { id: true },
