...
var sendResponse = (res, responseData) => {
  const { httpStatusCode, success, message, data, meta: meta2 } = responseData;
...
var getTestimonialsByExpert = async (expertId) => {
  ...
  if (!uuidRegex.test(expertId)) {
    ...
    if (!expertByEmail) {
      throw new AppError_default(status23.BAD_REQUEST, "Invalid expertId: must be a valid UUID or a valid expert email");
    }
...
var getStatsByExpertId = async (expertId, filters = {}) => {
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
  let realExpertId = expertId;
  if (!uuidRegex.test(expertId)) {
    const expertByEmail = await prisma.expert.findUnique({ where: { email: expertId } });
    if (!expertByEmail) {
      throw new AppError_default(status25.BAD_REQUEST, "Invalid expertId: must be a valid UUID or a valid expert email");
    }
    realExpertId = expertByEmail.id;
  }
  ...
  const bookingsByMonth = bookingsRaw.reduce((acc, curr) => {
  ...
  const earningsByMonth = earningsRaw.reduce((acc, curr) => {
    const month = curr.createdAt.toISOString().slice(0, 7);
...
var getExpertStats = async (user, filters = {}) => {
  const expert = await prisma.expert.findUniqueOrThrow({
    where: { userId: user.userId }
  });
  const consultationCount = await prisma.consultation.count({
...
var getClientStats = async (user, filters = {}, pagination = {}) => {
  const client = await prisma.client.findUnique({
    where: { userId: user.userId },
    select: { id: true }
  });
  ...
  const engagementTrends = engagementRaw.reduce((acc, curr) => {
    const month = curr.createdAt.toISOString().slice(0, 7);
    const found = acc.find((e) => e.month === month);
    if (found) found.count++;
    else acc.push({ month, count: 1 });
  ...
  const spendingByMonth = spendingRaw.reduce((acc, curr) => {
    const month = curr.createdAt.toISOString().slice(0, 7);
...
// src/modules/stats/stats.schemas.ts
import { z as z12 } from "zod";

// src/modules/stats/stats.validation.ts
import { z as z11 } from "zod";
var ExpertIdSchema = z11.string().refine(
  (val) => {
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
var AIInsightSchema = z11.object({
  id: z11.string().uuid(),
  type: z11.enum(["summary", "anomaly", "recommendation", "trend"]),
  title: z11.string(),
  description: z11.string(),
  confidence: z11.number().min(0).max(1),
  source: z11.enum(["ai", "human", "hybrid"]),
  createdAt: z11.string(),
  relatedIds: z11.array(z11.string()).optional(),
  meta: z11.record(z11.string(), z11.any()).optional()
});
var ExpertAnalyticsSchema = z11.object({
  id: z11.string().uuid(),
  name: z11.string(),
  spendingTrends: z11.array(z11.object({ date: z11.string(), amount: z11.number() })),
  engagement: z11.object({ sessions: z11.number(), messages: z11.number() }),
  categories: z11.array(z11.string()),
  aiInsights: z11.array(AIInsightSchema),
  meta: z11.any().optional()
});
var ClientAnalyticsSchema = z11.object({
  id: z11.string().uuid(),
  name: z11.string(),
  savedExperts: z11.array(z11.string()),
  engagement: z11.object({ sessions: z11.number(), messages: z11.number() }),
  spendingTrends: z11.array(z11.object({ date: z11.string(), amount: z11.number() })),
  aiInsights: z11.array(AIInsightSchema),
  meta: z11.any().optional()
});
var AdminAnalyticsSchema = z11.object({
  id: z11.string().uuid(),
  platformTrends: z11.array(z11.object({ date: z11.string(), value: z11.number() })),
  categoryBreakdown: z11.array(z11.object({ category: z11.string(), count: z11.number() })),
  aiInsights: z11.array(AIInsightSchema),
  meta: z11.any().optional()
});
var AIAnalyticsSchema = z11.object({
  id: z11.string().uuid(),
  usageTrends: z11.array(z11.object({ date: z11.string(), count: z11.number() })),
  feedback: z11.object({ likes: z11.number(), dislikes: z11.number() }),
  aiInsights: z11.array(AIInsightSchema),
  meta: z11.any().optional()
});

// src/modules/stats/stats.schemas.ts
var getStatsByExpertIdParamsSchema = z12.object({
...
var getStatsByExpertId2 = catchAsync(async (req, res) => {
...
var StatsController = {
  ...
  getStatsByExpertId: getStatsByExpertId2
};
...
import z13 from "zod";
var notificationIdParamsSchema = z13.object({
  id: z13.string().uuid("Invalid notification id")
});
var createNotificationValidation = z13.object({
  body: z13.object({
    type: z13.string().trim().min(1, "Type is required"),
    message: z13.string().trim().min(1, "Message is required"),
    userId: z13.string().uuid("Invalid user id").optional(),
    role: z13.nativeEnum(Role).optional()
...
var notificationIdValidation = z13.object({
...
var deleteMessage = async (roomId, messageId, userId, role) => {
  ...
  if (!message || message.senderId !== userId) {
    throw new AppError_default(httpStatus2.FORBIDDEN, "You can only delete your own messages");
  }
...
import z14 from "zod";
var toggleMessageReactionValidation = z14.object({
  params: z14.object({
    roomId: z14.string().min(1, "roomId is required"),
    messageId: z14.string().min(1, "messageId is required")
  }),
  body: z14.object({
    emoji: z14.string().trim().min(1, "Emoji is required").max(32, "Emoji is too long")
  })
});
...
var SYSTEM_PROMPT = `You are ConsultEdge AI, the intelligent assistant for the ConsultEdge platform\u2014a SaaS marketplace connecting clients and experts for professional consultations.
...
var generateOpenAIReply = async (payload) => {
  if (!envVars.OPENAI_API_KEY) {
    throw new AppError_default(status33.INTERNAL_SERVER_ERROR, "OpenAI API key is missing. AI features are disabled.");
  }
...
var askSupport = async (payload) => {
  ...
  if (!aiReply) {
    throw new AppError_default(status33.INTERNAL_SERVER_ERROR, "AI service unavailable. Please contact support.");
  }
  return {
    reply: aiReply,
    suggestedActions,
...
var CHAT_SYSTEM_PROMPT = `You are ConsultEdge AI, the intelligent assistant for the ConsultEdge platform\u2014a SaaS marketplace connecting clients and experts for professional consultations.
...
var industryCreation = async (input) => {
  ...
  try {
    const { data, meta: meta2 } = await aiProvider.generateJSON({
      messages: [
        {
          role: "system",
...
var sendAIResponse = (res, data, meta2, statusCode = 200) => {
  res.locals.aiMeta = meta2;
  return res.status(statusCode).json({ success: true, data, meta: meta2 });
};

// src/modules/ai/controllers/aiAdvanced.controller.ts
var recommendations2 = catchAsync(async (req, res) => {
  const payload = sanitizeObject(req.body);
  const { data, meta: meta2 } = await aiAdvancedService.recommendations(payload);
  sendAIResponse(res, data, meta2);
});
var industryCreation2 = catchAsync(async (req, res) => {
  const payload = sanitizeObject(req.body);
  payload.industryName = sanitizeText(payload.industryName, 100);
  const { data, meta: meta2 } = await aiAdvancedService.industryCreation(payload);
  sendAIResponse(res, data, meta2);
});
var search2 = catchAsync(async (req, res) => {
  const payload = sanitizeObject(req.body);
  payload.query = sanitizeText(payload.query, 500);
  const { data, meta: meta2 } = await aiAdvancedService.search(payload);
  sendAIResponse(res, data, meta2);
});
var summary2 = catchAsync(async (req, res) => {
  const payload = req.body;
  const { data, meta: meta2 } = await aiAdvancedService.summary({
...
var chat2 = catchAsync(async (req, res) => {
  ...
  const { data, meta: meta2 } = await aiAdvancedService.chat(payload);
  sendAIResponse(res, data, meta2);
});
var documentAnalysis2 = catchAsync(async (req, res) => {
  const payload = req.body;
  const { data, meta: meta2 } = await aiAdvancedService.documentAnalysis({
...
var sendMessage2 = catchAsync(async (req, res) => {
  const payload = sanitizeObject(req.body);
  const { data, meta: meta2 } = await aiChatService.sendMessage(req.user.userId, {
    message: sanitizeText(payload.message, 4e3),
    context: payload.context ? sanitizeText(payload.context, 500) : void 0,
    conversationId: payload.conversationId
  });
  sendAIResponse(res, data, meta2, httpStatus6.CREATED);
});
...
var profileSuggest = z16.object({
  body: z16.object({
    industry: z16.string().trim().min(2).max(100),
    feedback: z16.string().max(400).optional()
  })
});
var aiValidation = {
...
// src/modules/ai/services/aiProfileSuggest.service.ts
var aiProfileSuggestService = {
  async profileSuggest(input) {
...
// src/modules/ai/controllers/aiProfileSuggest.controller.ts
var profileSuggest2 = catchAsync(async (req, res) => {
  const payload = sanitizeObject(req.body);
  const { data, meta: meta2 } = await aiProfileSuggestService.profileSuggest(payload);
  sendAIResponse(res, data, meta2);
});
...
var aiLogger = (req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    const meta2 = res.locals.aiMeta ?? null;
    const ok = res.statusCode < 400;
    if (ok) {
      aiMetrics.recordSuccess(endpoint, durationMs, meta2?.tokensUsed ?? 0);
    } else {
    ...
    console.log(
      JSON.stringify({
        ...
        durationMs,
        provider: meta2?.provider ?? null,
        model: meta2?.model ?? null,
        tokensUsed: meta2?.tokensUsed ?? 0,
        modelLatencyMs: meta2?.latencyMs ?? null,
